// JUŻ NIE C, LECZ JESZCZE NIE C++
// namespace, using, std, cout, cin, new, delete, struct, przeładowanie funkcji

import <iostream>;  // odpowiednikiem w jęsyku C jest #include <stdio.h>
import <string>;
import<cstdlib>;   // nie używamy <cstdio>, ponieważ cstdlib ta biblioteka
...
//-------------------------------------------------------------------
// I.1. Użycie std:: . Używamy na razie formalnie, szczególowe wyjaśnienie później
//-------------------------------------------------------------------
void przyklad_1() {
...
void przyklad_3() {
    ...
    scanf_s("%s", _Osoba.godnosc, 100); // wartość godnosc nie może mieć spacji
...
//-------------------------------------------------------------------
// I.9. Pamięć dynamiczna (nizki posiom). Jednowymiarowa (1-D) Tablica obiektów w C++
//-------------------------------------------------------------------
...
void przyklad_9() {
    ...
    //std::srand(static_cast<unsigned int>(std::time(nullptr))); // Bład kompilacji w MS VS 22 (niedopracowane przez MS)
    //std::srand(std::time(0));                                  // Bład kompilacji w MS VS 22 (niedopracowane przez MS)
...
void ini(Osoba* const* const* const osoby, const size_t rows, const size_t cols) {
    for (size_t i = 0; i < cols; i++) {
        for (size_t j = 0; j < rows; j++) {
            gen_data(osoby[i][j]);
        }
...
void print(const Osoba* const* const* const osoby, const auto rows, const auto cols) {
    std::cout << "<======= Tablica, (" << rows << "," << cols << ") =======>" << std::endl;
    for (size_t j = 0; j < rows; j++) {
        for (size_t i = 0; i < cols; i++)
...
void przyklad_11() {
    Osoba*** plansza{nullptr};// każda kratka planszy skojarzona ze wskaźnikiem do Osoba
    size_t rows = 7;
    size_t cols = 10;
